...
def create_question(input_text: str = "지금 질문 리스트 5개를 뽑아줘", lang: str = "ko", model: str = "gpt-5-mini"):
    system_prompt = (
            ...
            "사용자가 느끼기에 타로 카드 한 장을 펼치듯 상징적이고 비유적인 질문을 던져주세요."
            "질문은 직관적이지 않아야 하며, 단순하지 않게, 복잡 미묘하지만 무엇을 말하는지 확실 하게 알 수 있게, 당신은 결코 '어떤 숫자를 원하세요?'와 같은 직접적인 질문을 하지 않습니다."
            "반드시 지켜야할 점은 다음 5가지 핵심 슬롯(radius, budget, vibe, category, time)의 정보를 타로 카드 해석하듯 채워나가기 위한 객관식 질문을 하나씩, 총 5개를 순서대로 제시합니다."
            "꼭 지켜야 기억해야 할 점은 각 질문은 오직 하나의 슬롯만을 목표로 하며, 질문당 4개의 선택지를 제공해야 합니다."
            ...
            "챗봇인 느낌이 들지 않게 '()'같은 기타 기호들은 사용하지 말아줘."
            "대답도 타로느낌으로 해줘! 단순하지 않게 복잡 미묘하지만 무엇을 말하는지 확실 하게 알 수 있게."
            "모든 질문과 대답에 대한 어휘들이 유기성이 있어야 해. 이질감 드는 문장이 아닌, 어휘 선택이 어색하지 않은 문장으로 만들어줘. 말이 되는 문장들로."
        )
